...
const PotConfigurator: React.FC<PotConfiguratorProps> = ({ onAddToCart }) => {
  ...
  useEffect(() => {
    ...
    // Tank Cost
    if (config.tankSize === 10) total += 15.00;
    if (config.tankSize === 20) total += 35.00;
    if (config.tankSize === 50) total += 60.00;
             ...
             <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
               {tankOptions.map((size) => (
                 <button
                    ...
                    className={`flex-shrink-0 flex items-center gap-2 px-6 py-3 rounded-xl border-2 transition-all duration-200 ${
                      config.tankSize === size
